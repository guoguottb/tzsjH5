		...
		<main>
			<div id="page1" v-show="page1" style="height: 600px; display: block;text-align: left;">
				...
				<van-form @submit="next1">
					...
					<van-field v-if="typelx!='特困供养'" name="ab_ex46" label="原因分类" :rules="[{ required: true, message: '请选择原因分类' }]">
						<template #input>
					...
					<van-field v-if="typelx!='特困供养'" name="ab_ex41" label="低保类型" :rules="[{ required: true, message: '请选择低保类型' }]">
						<template #input>
					...
					<van-field v-if="typelx!='特困供养'" name="ab_ex92" label="是否属于亲属" :rules="[{ required: true, message: '请选择是否属于亲属' }]">
						<template #input>
					...
					<!-- 特困 -->
					<van-field v-if="typelx=='特困供养'" v-model="form.ab_ex46_" center name="ab_ex46_" label="致贫原因" placeholder="致贫原因"
						:rules="[{ required: true, message: '请输入致贫原因' }]">
					</van-field>
					<van-field v-if="typelx=='特困供养'" name="ab_ex71" label="申请供养方式" :rules="[{ required: true, message: '请选择申请供养方式' }]">
						<template #input>
							<van-radio-group v-model="form.ab_ex71" direction="horizontal">
			...
			<div id="page2" v-show="page2" class="stepPage" style="display: block;padding-top: 20px;">
					...
					<van-form style="margin-top: 20px;">
						<div v-for="(item,index) in family" :key='index'>
							...
							<van-field v-model="item.abm_ex2" type="number" name="abm_ex2" label="身份证号"
								placeholder="身份证号" :rules="[{ required: true, message: '请填写身份证号'}]" @blur = 'changeSfz2(item)' />
							...
							<van-field v-if="typelx=='特困供养'" v-model="item.abm_ex29" type="number" name="abm_ex29" label="年龄"
								placeholder="年龄" :rules="[{ required: true, message: '请填写年龄'}]" />
							...
							<van-field v-if="typelx!='特困供养'" readonly clickable name="abm_ex52" :value="item.abm_ex52" label="重病病种"
							...
							<van-field v-if="typelx!='特困供养'" readonly clickable name="abm_ex51" :value="item.abm_ex51" label="特定对象"
							...
							<van-field v-if="typelx!='特困供养'" readonly clickable name="abm_ex14" :value="item.abm_ex14" label="职业状况"
							...
							<van-field v-if="typelx!='特困供养'" v-model="item.abm_ex15" name="abm_ex15" label="工作单位" placeholder="工作单位"
								:rules="[{ required: true, message: '请填写工作单位'}]" />
							<van-field v-if="typelx!='特困供养'" v-model="item.abm_ex17" type="number" name="abm_ex17" label="月收入"
								placeholder="月收入" :rules="[{ required: true, message: '请填写月收入'}]" />

							<van-field v-if="typelx!='特困供养'" readonly clickable name="abm_ex8" :value="item.abm_ex8" label="增发类型"
							...
							<van-field v-if="typelx=='特困供养'" readonly clickable name="abm_ex30" :value="item.abm_ex30" label="自理能力"
								placeholder="点击选择自理能力" @click="item.showPicker215 = true" />
							<van-popup v-model="item.showPicker215" position="bottom">
								<van-picker show-toolbar :columns="columns17" @confirm="onConfirm215($event,item)"
									@cancel="item.showPicker215 = false" />
							</van-popup>
							
							<van-field v-if="typelx=='特困供养'" readonly clickable name="abm_ex49" :value="item.abm_ex49" label="劳动能力"
								placeholder="点击选择劳动能力" @click="item.showPicker216 = true" />
							<van-popup v-model="item.showPicker216" position="bottom">
								<van-picker show-toolbar :columns="columns18" @confirm="onConfirm216($event,item)"
									@cancel="item.showPicker216 = false" />
							</van-popup>
							
							<van-field v-if="typelx=='特困供养'" readonly clickable name="abm_ex50" :value="item.abm_ex50" label="生活来源"
								placeholder="点击选择生活来源" @click="item.showPicker217 = true" />
							<van-popup v-model="item.showPicker217" position="bottom">
								<van-picker show-toolbar :columns="columns19" @confirm="onConfirm217($event,item)"
									@cancel="item.showPicker217 = false" />
							</van-popup>
							
							<van-field v-if="typelx=='特困供养'" readonly clickable name="abm_ex32" :value="item.abm_ex32" label="赡抚养人"
								placeholder="点击选择赡抚养人" @click="item.showPicker218 = true" />
							<van-popup v-model="item.showPicker218" position="bottom">
								<van-picker show-toolbar :columns="columns20" @confirm="onConfirm218($event,item)"
				...
				<div style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					...
					<van-form style="margin-top: 20px;">
						<div v-for="(item,index) in family2" :key='index'>
							...
							<van-field v-if="typelx!='特困供养'" v-model="item.abm_ex21" type="number" name="abm_ex21" label="家庭月总收入"
								placeholder="家庭月总收入" :rules="[{ required: true, message: '请填写家庭月总收入'}]" />
							<van-field v-if="typelx!='特困供养'" v-model="item.abm_ex20" type="number" name="abm_ex20" label="家庭总人口"
								placeholder="家庭总人口" :rules="[{ required: true, message: '请填写家庭总人口'}]" />
							<van-field v-if="typelx!='特困供养'" v-model="item.abm_ex18" type="number" name="abm_ex18" label="月赡（扶、抚）养费"
								placeholder="月赡（扶、抚）养费" :rules="[{ required: true, message: '请填写月赡（扶、抚）养费'}]" />
							
							<van-field v-if="typelx=='特困供养'" v-model="item.abm_ex9" name="abm_ex9" label="住址"
								placeholder="住址" :rules="[{ required: true, message: '请填写住址'}]" />
							<van-field v-if="typelx=='特困供养'" v-model="item.abm_ex10" name="abm_ex10" label="联系电话"
				...
				<!-- 监护人情况 -->
				<div v-if="typelx=='特困供养'" style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;" >
					<div style="border-bottom:#aaa solid 1px;font-size: 16px;">
						<span style="background-color: #00AAFF;display: inline-block;width: 5px;height: 10px;"></span>
					...
					<van-form style="margin-top: 20px;">
						<div v-for="(item,index) in family3" :key='index'>
							...
							<van-field readonly clickable name="abm_ex1" :value="item.abm_ex1" label="与户主关系"
								placeholder="点击选择与户主关系" @click="item.showPicker21 = true" />
							<van-popup v-model="item.showPicker21" position="bottom">
								<van-picker show-toolbar :columns="columns21" @confirm="onConfirm21($event,item)"
									@cancel="item.showPicker21 = false" />
							</van-popup>
							...
							<van-fiel v-model="item.abm_ex9" name="abm_ex9" label="住址"
								placeholder="住址" :rules="[{ required: true, message: '请填写住址'}]" />
							<van-field v-model="item.abm_ex10" name="abm_ex10" label="联系电话"
								placeholder="联系电话" :rules="[{ required: true, message: '请填写联系电话'}]" />
			...
			<div id="page3" v-show="page3" class="stepPage" style="display: block;padding-top: 20px;">
				<div style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					...
					<van-form style="margin-top: 20px;">
						<van-field v-if="typelx!='特困供养'" v-model="form.ab_ex29" name="ab_ex29" label="工资性收入" placeholder="工资性收入"
							:rules="[{ required: true, message: '请填写工资性收入' }]" type="number" @input='changeAmt'>
						</van-field>

						<van-field v-if="typelx!='特困供养'" v-model="form.ab_ex30" name="ab_ex30" label="经营净收入" placeholder="经营净收入"
							:rules="[{ required: true, message: '请填写经营净收入' }]" type="number" @input='changeAmt'>
						</van-field>
						<van-field v-if="typelx!='特困供养'" v-model="form.ab_ex31" name="ab_ex31" label="财产净收入" placeholder="财产净收入"
							:rules="[{ required: true, message: '请填写财产净收入' }]" type="number" @input='changeAmt'>
						</van-field>
						<van-field v-if="typelx!='特困供养'" v-model="form.ab_ex32" name="ab_ex32" label="转移净收入" placeholder="转移净收入"
							:rules="[{ required: true, message: '请填写转移净收入' }]" type="number" @input='changeAmt'>
						</van-field>
						<van-field v-if="typelx!='特困供养'" v-model="form.ab_ex33" name="ab_ex33" label="其他收入" placeholder="其他收入"
						...
						<van-field v-if="typelx!='特困供养'" v-model="form.ab_ex94" name="ab_ex94" label="家庭月平均收入" placeholder="家庭月平均收入"
							:rules="[{ required: true, message: '请填写家庭月平均收入' }]" type="number">
						</van-field>
						<!-- 应该是方法算出来的不是自己输入的 -->
						<van-field v-if="typelx!='特困供养'" v-model="form.ab_ex94_" name="ab_ex94_" label="家庭月人均收入" placeholder="家庭月人均收入"
							:rules="[{ required: true, message: '请填写家庭月人均收入' }]" type="number">
						</van-field>
						<van-field v-model="form.ab_ex38" name="ab_ex38" :label="typelx!='特困供养'?'低保标准':'生活补贴标准'" placeholder="低保标准"
							:rules="[{ required: true, message: '请填写低保标准' }]" type="number">
						</van-field>
						<van-field  v-model="form.ab_ex40" name="ab_ex40" :label="typelx!='特困供养'?'家庭月保障金额':'生活补贴'" placeholder="家庭月保障金额"
	...
	export default {
		data() {
			return {
				...
				columns21: ['父母', '配偶','子女','兄弟姐妹','(外)祖父母','(外)孙子女','其他'],
				family2: [{
					abm_ex3: '',
					abm_ex1: '',
					showPicker21: false,
		...
		methods: {
			...
			onConfirm215(value, item) {
				item.abm_ex30 = value;
				item.showPicker215 = false;
			},
			onConfirm216(value, item) {
				item.abm_ex49 = value;
				item.showPicker216 = false;
			},
			onConfirm217(value, item) {
				item.abm_ex50 = value;
				item.showPicker217 = false;
			},
			onConfirm218(value, item) {
				item.abm_ex32 = value;
				item.showPicker218 = false;
			},
			...
			save() {
				...
				console.log(this.form)
				let memeber = []
				// 可能需要加入abm_ex6为  Y,N,A
				if(this.typelx == '特困供养'){
					memeber = this.family.concat(this.family2).concat(this.family3)
				}else{
