...
<template>
	<div class="dbjz">
		...
		<main>
			<div id="page1" v-show="page1" style="margin-bottom: 100px;display: block;text-align: left;">
				...
				<van-form ref='rule1' @submit="next1">
					...
					<van-field v-model="form.ab_ex3" required name="ab_ex3" label="姓名" placeholder="姓名"
					...
					<van-field v-model="form.ab_ex28" center name="ab_ex28" type="number" required label="联系电话" placeholder="联系电话"
					...
					<van-field name="ab_ex145" label="求助属实类别" required :rules="[{ required: true, message: '请选择求助属实类别' }]">
						<template #input>
							<van-checkbox-group v-model="ab_ex145" direction="horizontal"  @change="sslbChange">
								...
								<van-checkbox :disabled="sslbDisabled6" class="marginstyle" name="家庭边缘" shape="square">低收入家庭</van-checkbox>
            ...
            <van-field v-model="form.ab_ex178" name="ab_ex178" type="textarea" autosize :rows="2" label="退回描述" placeholder="退回描述" />
						<van-cell title="*证明材料"></van-cell>
						<van-uploader v-model="fileList10" :before-read="beforeRead" :after-read="uploadImg(10)" />
						<van-cell title="点击提交放弃救助------>" center>
							<template>
								<van-button type="info" native-type="button" @click = "abandon">提交放弃</van-button>
							</template>
						</van-cell>
					</van-cell-group>
					<van-field name="ab_ex46" label="原因分类" required :rules="[{ required: true, message: '请选择原因分类' }]">
						<template #input>
					...
					<van-field name="ab_ex92" label="是否属于村干部或近亲属" required :rules="[{ required: true, message: '请选择是否属于村干部或近亲属' }]">
						<template #input>
					...
					<van-field v-if="typelx=='临时救助' && isZjfw ==true" name="ab_ex106" label="转介方向" required
						:rules="[{ required: true, message: '请选择转介方向' }]">
						<template #input>
					...
					<van-field v-model="form.ab_ex42" name="ab_ex42" label="开户银行" placeholder="开户银行" required
						:rules="[{ required: true, message: '请填写开户银行' }]">
						<!-- <template #button>
							<van-button size="small" type="info" native-type="button">拍照识别</van-button>
						</template> -->
					</van-field>
					<van-field v-model="form.ab_ex8" center name="ab_ex8" label="开户人" placeholder="开户人" required
						:rules="[{ required: true, message: '请填写开户人' }]">
					</van-field>
					<van-field v-model="form.ab_ex43" center name="ab_ex43" type="number" label="银行账号" placeholder="银行账号" required
					...
					<van-cell-group v-for="(item,index) in warning" :key='index'>
						<van-field name="message" v-model="item.aw_ex1" rows="1" autosize label="预警内容" type="textarea"
							placeholder="请输入预警内容" />
					...
					<div style="width: 100%;position: fixed;bottom: 20px;text-align: center;display: flex;justify-content: space-around">
						<van-button round type="info" native-type="button" @click="next1">下一步</van-button>
					</div>
				</van-form>
			</div>

			<div id="page2" v-show="page2" class="stepPage" style="margin-bottom: 100px;display: block;padding-top: 20px;">
				<div style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					...
					<van-form ref='rule2' style="margin-top: 20px;">
						...
						<!-- 	<van-field readonly clickable name="abm_ex48" :value="item.abm_ex48" label="年龄段" required
							...
							<van-field readonly clickable name="abm_ex46" :value="item.abm_ex46" label="是否属于劳动年龄" required
							...
							<van-field v-if="item.abm_ex11 =='重病'" readonly clickable name="abm_ex52" required
							...
							<van-field v-if="item.abm_ex11 =='残疾'" readonly clickable name="abm_ex39" required
							...
							<van-field v-model="item.abm_ex15" required name="abm_ex15" label="工作单位" placeholder="工作单位" />
						...
						<!-- 	<van-field v-if="typelx!='特困供养' && typelx!='临时救助'" readonly clickable name="abm_ex8"
							...
							<!-- <van-field v-if="typelx=='特困供养'" readonly required clickable name="abm_ex49" :value="item.abm_ex49"
				...
				<div style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					...
					<van-form ref="rule3" style="margin-top: 20px;">
						<div v-for="(item,index) in family2" :key='index'>
							...
							<van-field v-model="item.abm_ex15" name="abm_ex15" label="工作单位" required placeholder="工作单位" />
							<van-field v-model="item.abm_ex17" type="number" required name="abm_ex17" label="家庭人均月收入"
								placeholder="家庭人均月收入" :rules="[{ required: true, message: '请填写家庭人均月收入'}]"
							...
							<van-field v-model="item.abm_ex20" type="number"
								name="abm_ex20" label="家庭总人口" placeholder="家庭总人口" required
								:rules="[{ required: true, message: '请填写家庭总人口'}]" />
							<van-field v-model="item.abm_ex18" type="number"
								name="abm_ex18" label="月赡（扶、抚）养费" placeholder="月赡（扶、抚）养费" required
				...
				<div style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					<div style="border-bottom:#aaa solid 1px;font-size: 16px;">
						<span style="background-color: #00AAFF;display: inline-block;width: 5px;height: 10px;"></span>
						<span style="color: #00AAFF;">非共同生活赡抚养人家庭成员信息</span>
					</div>
				...
				<!-- <div v-if="typelx=='特困供养'"
					style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					<div style="border-bottom:#aaa solid 1px;font-size: 16px;">
						<span style="background-color: #00AAFF;display: inline-block;width: 5px;height: 10px;"></span>
						<span style="color: #00AAFF;">监护人情况</span>
					</div>
					<van-form ref='rule4' style="margin-top: 20px;">
			...
			<!-- 家庭前12个月总收入情况 -->
			<div id="page3" v-show="page3" class="stepPage" style="margin-bottom: 100px;display: block;padding-top: 20px;">
				<div style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					<div style="border-bottom:#aaa solid 1px;font-size: 16px;">
						<span style="background-color: #00AAFF;display: inline-block;width: 5px;height: 10px;"></span>
					...
					<van-form style="margin-top: 20px;">
						...
						<van-field v-if="typelx!='临时救助' && typelx!='家庭边缘' && typelx!='支出型困难'" v-model="form.ab_ex40" name="ab_ex40"
							:label="typelx!='特困供养'?'家庭月保障金额':'生活补贴'" :placeholder="typelx!='特困供养'?'家庭月保障金额':'生活补贴'"
							:rules="[{ required: true, message: '请填写家庭月保障金额(生活补贴)' }]" type="number">
						</van-field>
				...
				<div style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					<div style="border-bottom:#aaa solid 1px;font-size: 16px;">
						<span style="background-color: #00AAFF;display: inline-block;width: 5px;height: 10px;"></span>
						<span style="color: #00AAFF;">家庭前12个月总支出情况</span>
					</div>
					<van-form style="margin-top: 20px;">
						<van-field v-model="pay.abp_ex4" name="abp_ex4" label="医疗支出"
							placeholder="医疗支出" :rules="[{ required: true, message: '请填写医疗支出' }]" type="number">
						</van-field>
						<van-field v-model="pay.abp_ex5" name="abp_ex5" label="教育支出"
							placeholder="教育支出" :rules="[{ required: true, message: '请填写教育支出' }]" type="number">
						</van-field>
						<van-field v-model="pay.abp_ex6" name="abp_ex6" label="残疾康复费用支出"
							placeholder="残疾康复费用支出" :rules="[{ required: true, message: '请填写残疾康复费用支出' }]" type="number">
						</van-field>
						<van-field v-model="pay.abp_ex7" name="abp_ex7" label="其他支出"
							placeholder="其他支出" :rules="[{ required: true, message: '请填写其他支出' }]" type="number">
						</van-field>
						<van-field v-model="pay.abp_ex8" name="abp_ex8" label="家庭总支出"
							placeholder="家庭总支出" :rules="[{ required: true, message: '请填写家庭总支出' }]" type="number">
						</van-field>
						<van-field v-model="pay.abp_ex9" name="abp_ex9" label="扣减刚性支出后家庭年收入"
							placeholder="扣减刚性支出后家庭年收入" :rules="[{ required: true, message: '请填写扣减刚性支出后家庭年收入' }]" type="number">
						</van-field>
						<van-field v-model="pay.abp_ex10" name="abp_ex10" label="扣减刚性支出后人均年收入"
							placeholder="扣减刚性支出后人均年收入" :rules="[{ required: true, message: '请填写扣减刚性支出后人均年收入' }]" type="number">
						</van-field>
						<div style="border-bottom: 10px solid #f3f3f3;"> </div>
					</van-form>
				</div>
				...
				<div v-if="typelx=='临时救助'"
					...
					<div style="border-bottom:#aaa solid 1px;font-size: 16px;">
						<span style="background-color: #00AAFF;display: inline-block;width: 5px;height: 10px;"></span>
						<span style="color: #00AAFF;">救助核算</span>
					</div>
					<van-form style="margin-top: 20px;">
			...
			<!-- 财产情况 -->
			<div id="page4" v-show="page4" class="stepPage" style="margin-bottom: 100px;display: block;padding-top: 20px;">
				...
				<div style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					...
					<van-form style="margin-top: 20px;">
						<van-field v-model="form.ab_ex36" name="ab_ex36" type="number" label="银行总存款" placeholder="银行总存款"
					...
					<van-form style="margin-top: 20px;">
						<div v-for="(item,index) in estate" :key='index'>
							...
							<van-popup v-model="item.showPicker43" position="bottom">
								<van-datetime-picker v-model="item.currentDate" type="date" @confirm="onConfirm43($event,item)"
									@cancel="item.showPicker43 = false" />
							</van-popup>
							
							<van-field v-model="item.abe_ex3" name="abe_ex3" label="用途" placeholder="用途"
				...
				<div style="display:flex;flex-direction:column;justify-content: center;align-items: center;">
					<div style="overflow: auto;margin-top:20px;text-align: left;width:80%;height:250px;font-size:12px;color:#545454;padding-left:20px;padding-right:20px;border-radius: 15px;border:#aaa solid 1px;">
						<div style="height:40px;line-height:40px;font-size: 17px;">房屋整体照片:</div>
						<div style="margin-top:10px;">
							<div id="zmcl" style="position:relative;width:91%;float: left;">
								<van-uploader v-model="fileList6" :before-read="beforeRead" :after-read="uploadImg(6)" />
							</div>
						</div>
					</div>
					<div style="overflow: auto;margin-top:20px;text-align: left;width:80%;height:250px;font-size:12px;color:#545454;padding-left:20px;padding-right:20px;border-radius: 15px;border:#aaa solid 1px;">
						<div style="height:40px;line-height:40px;font-size: 17px;">房屋室内照片:</div>
						<div style="margin-top:10px;">
							<div id="zmcl" style="position:relative;width:91%;float: left;">
								<van-uploader v-model="fileList7" :before-read="beforeRead" :after-read="uploadImg(7)" />
							</div>
						</div>
					</div>
				</div>
				<div style="display:flex;flex-direction:column;justify-content: center;align-items: center;">
					<div style="overflow: auto;margin-top:20px;text-align: left;width:80%;height:250px;font-size:12px;color:#545454;padding-left:20px;padding-right:20px;border-radius: 15px;border:#aaa solid 1px;">
						<div style="height:40px;line-height:40px;font-size: 17px;">房产材料照片:</div>
						<div style="margin-top:10px;">
							<div id="zmcl" style="position:relative;width:91%;float: left;">
								<van-uploader v-model="fileList8" :before-read="beforeRead" :after-read="uploadImg(8)" />
							</div>
						</div>
					</div>
					<div style="overflow: auto;margin-top:20px;text-align: left;width:80%;height:250px;font-size:12px;color:#545454;padding-left:20px;padding-right:20px;border-radius: 15px;border:#aaa solid 1px;">
						<div style="height:40px;line-height:40px;font-size: 17px;">车船材料照片:</div>
						<div style="margin-top:10px;">
							<div id="zmcl" style="position:relative;width:91%;float: left;">
								<van-uploader v-model="fileList9" :before-read="beforeRead" :after-read="uploadImg(9)" />
							</div>
						</div>
					</div>
				</div>
			...
			<div id="page5" v-show="page5" class="stepPage" style="margin-bottom: 100px;display: block;padding-top: 20px;">
				<div style="font-size:26px;padding-left:20px;padding-right:20px;text-align: left;">
					<div style="margin-bottom: 10px;">授权书编号</div>
					<!-- <td v-for="(item,index) in form.ab_ex85.split('')">{{item}}</td> -->
					<van-field v-model="form.ab_ex85" type="number" size="large"></van-field>
			...
			<div id="page6" v-show="page6" class="stepPage" style="margin-bottom: 100px;padding-left: 10px; display: none;">
				<div style="font-size:12px;text-align: left;">
					<div style="border-bottom:#aaa solid 1px;font-size: 16px;">
						<span style="background-color: #00AAFF;display: inline-block;width: 5px;height: 10px;"></span>
						<span style="color: #00AAFF;">照片</span>
					</div>
					<div style="width:80%;margin-left:3%;height:200px;font-size:16px;color:#545454;padding-left:20px;padding-right:20px;margin-top:10px;border-radius: 15px;border:#aaa solid 1px;">
						...
						<div style="margin-top:10px;">
							<div style="position:relative;width:100%;float: left;">
								<div style="width:40%;height:200px;text-align: center;float: left;margin-left:20px;"
									...
									<van-uploader v-model="fileList" :before-read="beforeRead" :max-count="1" :after-read="uploadImg(1)" />
								...
								<div style="width:40%;height:200px;text-align: center;float: right;margin-right:20px;"
									class="cd-bouncy-nav-trigger">
									<van-uploader v-model="fileList2" :before-read="beforeRead" :max-count="1" :after-read="uploadImg(2)" />
									<br>
									<span>上传身份证反面</span>
								</div>
							</div>
					...
					<div style="width:80%;margin-left:3%;height:200px;font-size:16px;color:#545454;padding-left:20px;padding-right:20px;margin-top:10px;border-radius: 15px;border:#aaa solid 1px;">
						...
						<div style="margin-top:10px;">
							<div style="position:relative;width:100%;float: left;">
								<div style="width:40%;height:200px;text-align: center;float: left;margin-left:20px;"
									class="cd-bouncy-nav-trigger">
									<van-uploader v-model="fileList3" :before-read="beforeRead" :max-count="1" :after-read="uploadImg(3)" />
								...
								<div style="width:40%;height:200px;text-align: center;float: right;margin-right:20px;"
									...
									<van-uploader v-model="fileList4" :before-read="beforeRead" :max-count="1" :after-read="uploadImg(5)" />
									<br>
									<span>民主评议</span>
								</div>
							</div>
				...
				<div style="overflow: auto;text-align:left;margin-left:3%;width:80%;height:250px;font-size:12px;color:#545454;padding-left:20px;padding-right:20px;border-radius: 15px;border:#aaa solid 1px;">
					<div style="height:40px;line-height:40px;font-size: 17px;">其他证明材料:</div>
					<div style="margin-top:10px;">
						<div id="zmcl" style="position:relative;width:91%;float: left;">
							<van-uploader v-model="fileList5" :before-read="beforeRead" :after-read="uploadImg(4)" />
						</div>
					</div>
				</div>
				<!-- 核查结果 -->
				...
				<div style="font-size:12px;padding-left:20px;padding-right:20px;text-align: left;">
					...
					<van-form style="margin-top: 20px;">
						...
						<van-field v-model="form.ab_ex78" name="ab_ex78" label="调查人" placeholder="调查人" required
							:rules="[{ required: true, message: '请填写调查人' }]">
						</van-field>

						<van-field v-model="form.ab_ex80" name="ab_ex80" label="备注" placeholder="备注" required
...
<script>
	...
	import {xzqh,compressImg} from '@/utils/utils.js'
	...
	export default {
        ...
        isShow: true,
				...
				columns12: ['是', '否'],
				columns13: ['','老年人', '未成年人','独居', '归侨','散居归侨侨属','少数民族','重病', '重残','退役军人','军转干部','已故原工商业者无工作的配偶','艾滋病患者', '易肇事肇祸精神病人', '两牢释放人员', '社区矫正人员', '建国前老党员','高校毕业生','宗教教职人员'],
				// columns14: ['自有私房', '租用公房', '租用私房', '临时搭建房', '借用房', '其他'],
				// columns15: ['自购房', '自建房', '回迁房', '承租公房', '其他'],
				columns16: ['属实', '不属实', '转其他救助'],
				// columns17: ['自理', '失能', '半失能'],
				// columns18: ['60岁以上，无劳动能力', '18岁以下，无劳动能力', '智力精神一二级，无劳动能力', '肢体一级，无劳动能力', '其他'],
				...
				columns25: ['户主本人','父母', '配偶', '子女', '兄弟姐妹','孙子女','其他'],
				columns26: ['健康', '重病', '重残'],
				columns27: ['父母', '配偶', '子女', '兄弟姐妹','其他'],
				family2: [{
					abm_ex3: '',
					abm_ex1: '',
					showPicker21: false,
        ...
        // 民主评议可以不传
				...
				let memeber = this.family.concat(this.family2).concat(this.family4)
					...
					if(this.family.length == 0){
						...
						for(let i =0;i<this.family.length;i++){
							this.$refs.rule2.validate().then(()=>{
								...
								if(this.isCardID(this.family[i].abm_ex2)){
									this.$toast.fail('共同家庭生活成员第'+parseInt(i+1)+'位'+this.isCardID(this.family[i].abm_ex2))
									resolve(false)
								}
								...
								// }
									...
									if(hz ==0){
										this.$toast.fail('共同家庭生活成员请添加户主本人')
										resolve(false)
									}else{
										console.log('返回true')
										resolve(true)
					...
					if(this.family2.length == 0){
						...
						for(let i =0;i<this.family2.length;i++){
							...
							this.$refs.rule3.validate().then(()=>{
								if(this.isCardID(this.family2[i].abm_ex2)){
									this.$toast.fail('非共同家庭生活成员第'+parseInt(i+1)+'位'+this.isCardID(this.family2[i].abm_ex2))
									resolve(false)
								}
								// else if(!this.family2[i].resultImg){
								// 	this.$toast.fail('非共同家庭生活成员第'+parseInt(i+1)+'位请签名')
								// 	resolve(false)
								// }
								...
								this.$toast.fail('非共同家庭生活成员第'+parseInt(i+1)+'位'+err[0].message)
								resolve(false)
							})
						}
			...
			// 					if(this.isCardID(this.family3[i].abm_ex2)){
			// 						this.$toast.fail('监护人成员第'+parseInt(i+1)+'位'+this.isCardID(this.family3[i].abm_ex2))
			// 						resolve(false)
				...
				return new Promise(resolve=>{
					if(this.family4.length == 0){
						...
						for(let i =0;i<this.family4.length;i++){
							this.$refs.rule4.validate().then(()=>{
								if(this.isCardID(this.family4[i].abm_ex2)){
									this.$toast.fail('非共同生活赡抚养人第'+parseInt(i+1)+'位'+this.isCardID(this.family4[i].abm_ex2))
									resolve(false)
								}else if(i == this.family4.length-1){
									resolve(true)
								}
							}).catch(err=>{
								this.$toast.fail('非共同生活赡抚养人第'+parseInt(i+1)+'位'+err[0].message)
								resolve(false)
							})
						}
			...
			next2() {
				let next2f = async ()=>{
					...
					if(await this.familyValidate() && await this.family2Validate() && await this.family4Validate() ){
						this.hold()
						this.page2 = false
						this.page3 = true
						this.isShow2 = false
      ...
      sslbChange(val){
        ...
        this.form.ab_ex178 = ''
			...
			changeSfz2(item) {
				console.log(item)
				if (this.isCardID(item.abm_ex2) == '') {
					item.abm_ex5 = this.getSex(item.abm_ex2)
					item.abm_ex29 = this.GetAge(item.abm_ex2)
					if(item.abm_ex5)
					if(item.abm_ex5 == "男"){
						if(item.abm_ex29 >= 60){
