<template>
	<hc :headername='"残疾人两项补贴"' :jz='"cjrjz"'></hc>
</template>
				...
				<van-list v-model="loading" :finished="finished" finished-text="没有更多了" @load="onLoad">
					<div class="hc" v-for="(item,index) in items" :key='index' @click="routerPush(item.ab_id)">
						...
						<div class="hcHeader2">
							...
							<div style="margin-left: 10px;">性别：{{item.ab_ex69}}</div>
						</div>
						<div class="hcHeader3">住所地：{{item.ab_ex129}}</div>
						<div class="hcHeader4">申请日期：{{item.ab_dat14}}</div>
					</div>
				</van-list>
			</van-pull-refresh>
		</main>
		<van-button style="width: 90%;position: fixed;bottom: 20px;" type="info" block :to="'cjrjz?sId=&type=残疾人两项补贴&ab_ex100=Y'">主动救助</van-button>
	</div>
</template>
	...
	export default {
		...
		methods: {
			loadData(data) {
				return request({
					method: 'post',
					url: '/social/assistanceWeixin/FLListByPage?stype='+this.$route.query.stype,
					data,
					headers: {
			...
			onLoad() {
				...
				this.loadData({
					page: this.page,
					ab_ex3:this.$route.query.ab_ex3,
					ab_ex7:this.$route.query.ab_ex7
					// 'sqlwhere': ""
				}).then(res => {
					// 关闭刷新
					...
					for (let i = 0; i < res.length; i++) {
						...
						this.$set(this.items, num, {
							...
							ab_ex129: unescape(res[i].ab_ex129),
							ab_ex69: unescape(res[i].ab_ex69),
